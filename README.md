...
crates/
  ...
  framkey-btc               BTC address/network/account boundary types
...
CH347T SPI NOR ROM workflow:

```bash
cargo run -p framkey-cli -- device probe --device ch347
cargo run -p framkey-cli -- device read-save --device ch347 --expected-save-size 8388608 --out ch347-read.bin
cargo run -p framkey-cli -- device write-save --device ch347 --expected-save-size 8388608 --input ch347-read.bin
cargo run -p framkey-cli -- device verify-save --device ch347 --expected-save-size 8388608 --blake3 <hash>
```

The CH347T path invokes `flashrom` as a child process using argv, not a shell string, and defaults to flashrom's chip auto-detection. Add `--chip <flashrom-chip-name>` only when flashrom reports multiple candidates or cannot choose the chip automatically; use exact names from `flashrom -L`, such as `W25Q64JV-.Q` rather than the shorter marking printed on the package. Install flashrom 1.4 or newer with `ch347_spi` support, or pass `--flashrom <path>` to a compatible build. The optional `--spispeed` value is passed to flashrom's `ch347_spi:spispeed=` parameter and can be one of `60M`, `30M`, `15M`, `7.5M`, `3.75M`, `1.875M`, `937.5K`, or `468.75K`. The first CH347T slice treats the ROM as a full-chip opaque image; choose `--expected-save-size` to match the chip dump size before writes. `write-save` invokes flashrom's write operation and then performs an explicit fresh readback comparison before reporting success.

The desktop Safety workspace has the same CH347T write/readback guarantee for physical backup media: choose one recovery `backup-xx.dat` file or a prebuilt full-chip image, optionally set an absolute flashrom path, chip override, and SPI speed, then confirm the write. On macOS the desktop process does not run as root; it hashes the selected file, writes a private request file, and asks macOS to run only `framkey-ch347-helper` with administrator authorization. The helper re-reads the selected file, verifies the size and BLAKE3 hash, probes the connected ROM capacity through flashrom, and if the selected file is smaller than the chip wraps it into a FRAMKey physical-backup ROM image with `0xFF` padding before invoking flashrom through argv. A prebuilt full-chip image whose size matches the probed chip is written as-is. The helper writes once, then performs one fresh full-chip readback comparison. The same Safety panel can read the connected ROM into a selected output directory: if the ROM contains the FRAMKey physical-backup header, the helper verifies the embedded payload BLAKE3 and saves the original backup file; otherwise it saves the full-chip image. Returned metadata reports paths, payload size/hash, full ROM image size/hash, chip selection, speed, and verification flags without returning backup bytes, recovery share bytes, or wallet secret material.
