...
The trusted `main` UI is split into five workflow workspaces: Home, DeFi, Safety, Activity, and System. It has no app-level product header below the native macOS titlebar; the trusted body starts directly with workspace navigation and wallet content. Home keeps daily wallet status, portfolio, and trusted send actions together. DeFi is the consumer dApp cockpit: app choice, first-screen approval, current app, wallet access, next action, sites with wallet access, launch controls, the full pending-approval panel, and the latest transaction outcome. Safety owns CH347 ROM backup writing and reading, vault creation, backup placement, and restore. Activity owns post-approval outcomes, receipt tracking, persistence state, recovery guidance, and transaction history. System keeps provider events, compatibility checks, capabilities, raw command output, and lower-level readiness state. The selected workspace is stored in local WebView storage only. Home surfaces a pending-approval jump action when needed, but the full approval card is not duplicated across unrelated workspaces.
...
The trusted Safety workspace also includes a CH347T physical backup writer and reader. The user selects one local recovery `backup-xx.dat` file or a prebuilt full-chip image and explicitly confirms replacing the connected ROM before writes. The user can also choose an output directory and read the connected ROM back into a local file. These commands are restricted to the trusted `main` window. On macOS, the desktop process writes a private helper request, then asks macOS to run only `framkey-ch347-helper` with administrator authorization; the full desktop app is not run as root. The helper accepts only request/response file paths as process arguments, passes flashrom arguments through argv, defaults to flashrom chip auto-detection, and accepts an optional flashrom chip override only for ambiguous chips. For writes, it re-validates input size and BLAKE3 before hardware access, probes the connected chip capacity through flashrom, wraps smaller inputs into a FRAMKey physical-backup ROM image with a small header and `0xFF` padding, writes once, and performs one fresh full-chip readback comparison. For reads, it probes and reads the full ROM image, verifies the FRAMKey physical-backup header and embedded payload BLAKE3 when present, then saves the original backup payload to the selected output directory; ROMs without that header are saved as full-chip images. Returned output is limited to paths, payload size/hash, full ROM image size/hash, helper identity, chip selection metadata, and verification flags; it does not return backup bytes, recovery share bytes, or wallet secret material.
...
Pending approvals are owned by the DeFi workspace through the `Approvals` panel. The first pending request is also promoted into a first-screen approval card with user-intent title, consequence text, risk/impact badges, and approve/reject actions wired to the same review broker decision path. Home changes its app action when approvals are waiting so the user can navigate back to DeFi review without duplicating the full approvals card in Safety, Activity, or System. Method names, technical summary, and request data remain available as collapsible details instead of leading the confirmation.

The trusted Activity workspace records the post-review transaction lifecycle in local trusted desktop state. It starts when an `eth_sendTransaction` review is captured, updates when the user approves or rejects it, records successful broadcast hashes or signing/broadcast errors, and refreshes `eth_getTransactionReceipt` for recent broadcast hashes through the configured trusted RPC. The Activity top surface summarizes latest outcome, receipt/network status, local history persistence, and the next user action before showing the full transaction list. DeFi also mirrors the latest actionable outcome so the dApp flow stays understandable without leaving the DeFi tab. The UI automatically polls pending receipts on a bounded interval while refreshable transaction hashes exist, and still keeps manual `Refresh` and `Refresh Receipts` controls for diagnostics and transient state recovery. Sanitized activity entries are persisted to local app state and restored on restart, while provider events, review queue items, account grants, and dApp compatibility evidence remain process-local. The pending approvals panel clears only currently waiting trusted approvals; Activity has its own trusted `Clear history` action that removes the sanitized in-memory activity log and writes an empty local activity file. On Unix/macOS, the activity directory and JSON file are written owner-only. Restored `review_pending` or `approved` activities are marked expired because the trusted review queue itself is not persisted; the user must retry from the dApp to create a fresh approval. If the activity file is corrupt or unavailable, the app starts with an empty activity log and shows a sanitized persistence warning instead of blocking wallet startup. Each activity item also carries sanitized recovery guidance: live simulation failures point to RPC health or retrying simulation, insufficient-funds broadcast errors point to funding native gas on the active network, nonce conflicts point to refreshing pending activity, chain mismatches point to checking the active network, and reverted execution points back to refreshing dApp quote/allowance state. Receipt rendering keeps only a small sanitized subset: status, block number, transaction index, gas used, and effective gas price. It does not expose a dApp history method and does not store raw calldata, raw signed transactions, signatures, Alchemy credentials, wallet secrets, or recovery material.
...
Omit `device.port` for the normal desktop path. Add `"port": "/dev/cu.usbserial-..."` only when multiple matching adapters are attached or you need to pin a specific macOS device node.
