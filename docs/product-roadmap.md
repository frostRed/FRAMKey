...
- Added a multichain Wallet account surface: secp256k1 vaults now expose structured EVM, BTC mainnet, and BTC Testnet4 account entries, the trusted UI shows separate EVM and Bitcoin cards, Testnet4 is the default BTC test network, and BTC now supports trusted UI balance refresh plus controlled P2WPKH PSBT sends through explicit UTXO/fee/change review.
